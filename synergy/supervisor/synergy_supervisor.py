...
class Supervisor(SynergyProcess):
    ...
    def _kill_process(self, box_config):
        ...
        try:
            self.logger.info('kill: %s {' % box_config.process_name)
            ...
            self.logger.error('Exception on killing: %s' % box_config.process_name, exc_info=True)
        finally:
            self.logger.info('}')
    ...
    def _start_process(self, box_config):
        ...
        try:
            self.logger.info('start: %s {' % box_config.process_name)
            ...
            self.logger.info('Started %s with pid = %r' % (box_config.process_name, p.pid))
        except Exception:
            box_config.set_process_pid(box_config.process_name, None)
            self.logger.error('Exception on starting: %s' % box_config.process_name, exc_info=True)
        finally:
            self.bc_dao.update(box_config)
            self.logger.info('}')
    ...
    def _poll_process(self, box_config):
        ...
        try:
            ...
            if return_code is None:
                ...
                self.logger.info('Process %s is terminated' % box_config.process_name)
                return
            else:
                # process is terminated; possibly by OS
                box_config.pid = None
                self.bc_dao.update(box_config)
                self.logger.info('Process %s got terminated. Cleaning up' % box_config.process_name)
        except TimeoutExpired:
            # process is alive and OK
            pass
        except Exception:
            self.logger.error('Exception on polling: %s' % box_config.process_name, exc_info=True)
    ...
    def start(self, *_):
        ...
        try:
            ...
            for box_config in box_configurations:
                ...
                handler.start()
                self.logger.info('Started Supervisor for %s, triggering every %d seconds'
                                 % (box_config.process_name, TRIGGER_INTERVAL))
